...
real, allocatable :: sdf2(:,:)
real, allocatable :: szf2(:,:)
real, allocatable :: sdt2(:,:)
real, allocatable :: szt2(:,:)
...
real, allocatable :: dvdt(:,:)
real, allocatable :: gudt(:,:)
real, allocatable :: gvdt(:,:)
...
real, allocatable :: du2(:)     ! u * cos(phi)
real, allocatable :: dv2(:)     ! v * sin(phi)
...
real         ,allocatable :: sidalt(:) ! sid in alternative grid
...
allocate(sdf2(nesp,nlev))   ; sdf2(:,:)  = 0.0
allocate(szf2(nesp,nlev))   ; szf2(:,:)  = 0.0

allocate(sdt2(nspp,nlev))   ; sdt2(:,:)  = 0.0
allocate(szt2(nspp,nlev))   ; szt2(:,:)  = 0.0
...
allocate(du(nhor,nlev))   ; du(:,:)  = 0.0
allocate(dv(nhor,nlev))   ; dv(:,:)  = 0.0
allocate(dudt(nhor,nlev))   ; dudt(:,:)  = 0.0
allocate(dvdt(nhor,nlev))   ; dvdt(:,:)  = 0.0
allocate(gudt(nhor,nlev))   ; gudt(:,:)  = 0.0
allocate(gvdt(nhor,nlev))   ; gvdt(:,:)  = 0.0
...
allocate(du2(nhor))   ; du2(:)  = 0.0 ! u * cos(phi)
allocate(dv2(nhor))   ; dv2(:)  = 0.0 ! v * sin(phi)
...
call mpbci(nruido)
call initruido      ! set random seed and allocate ruido arrays
...
call mpscrn(sidalt,NLPP) ! real (kind=8)
...
!call mpscrn(gsinalt,NLPP) ! real (kind=8)
...
!     ***************************
      !nkits=30 ! debug!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
               ...
               write(nud,*) "time12",du2(3300),sz(2,2), nstep   !HALLOCYCLONE
 
      ikits = nkits
      do 1000 jkits=1,ikits
         deltsec  = (sid_day / ntspd) / (2**nkits)
         deltsec2 = deltsec + deltsec      
         delt  = (TWOPI/ntspd) / (2**nkits) 
         delt2 = delt + delt
!               write(nud,*) "inix1",gu(30,4),sz(2,2), nstep   !HALLOCYCLONE
         call gridpoint
!               write(nud,*) "inix2",gu(30,4),sz(2,2), nstep   !HALLOCYCLONE
         call makebm
!               write(nud,*) "inix3",gu(30,4),sz(2,2), nstep   !HALLOCYCLONE
         call spectral
!               write(nud,*) "inix4",gu(30,4),sz(2,2), nstep   !HALLOCYCLONE
         nkits = nkits - 1
 1000 continue

               write(nud,*) "time12",du2(3300),sz(2,2), nstep   !HALLOCYCLONE

      deltsec  = sid_day / ntspd   ! timestep in seconds
      deltsec2 = deltsec + deltsec   ! timestep in seconds * 2
      delt  = TWOPI/ntspd
      delt2 = delt + delt
      call makebm
               write(nud,*) "time13",du2(3300),sz(2,2), nstep   !HALLOCYCLONE      
!               write(nud,*) "time11",gu(30,4),sz(2,2), nstep   !HALLOCYCLONE
      nstep1 = nstep ! remember 1.st timestep
!               write(nud,*) "time12",gu(30,4),sz(2,2), nstep   !HALLOCYCLONE
      call cyclones2() ! initializes cyclone velocity du2 dv2
              write(nud,*) "time14",du2(3300),sz(2,2), nstep   !HALLOCYCLONE      
      do 2000 jstep=1,nrun
         nstep = nstep + 1
         call ntomin(nstep,ndatim(5),ndatim(4),ndatim(3),ndatim(2),ndatim(1))
              write(nud,*) "time15",du2(3300),sz(2,2), nstep   !HALLOCYCLONE      
         

!               write(nud,*) "time13",gu(30,4),sz(2,2), nstep   !HALLOCYCLONE
!        ************************************************************
!        * calculation of non-linear quantities in grid point space *
!        ************************************************************
!               write(nud,*) "time14",gu(30,4),sz(2,2), nstep   !HALLOCYCLONE
         call gridpoint


               write(nud,*) "time16",du2(3300),sz(2,2), nstep   !HALLOCYCLONE
...
!                        write(nud,*) "timex1",gu(30,4), nstep   !HALLOCYCLONE
...
!        ******************************
!               write(nud,*) "time16",gu(30,4),sz(2,2), nstep   !HALLOCYCLONE
         if (jstep>10) then
         call gridpointd
         endif
         !if (jstep>20) then
         ! nshutdown=1
         !endif
              write(nud,*) "time17",du2(3300),sz(2,2), nstep   !HALLOCYCLONE      
         call spectral
              write(nud,*) "time18",du2(3300),sz(2,2), nstep   !HALLOCYCLONE      
!               write(nud,*) "time17",gu(30,4),sz(2,2), nstep   !HALLOCYCLONE
...
!     ========================
      ...
      call ppp_def_i('NLAT',nlat_ppp,1)
      call ppp_def_i('NLEV',nlev_ppp,1)

      call ppp_def_r('SIGMH',sigmh,nlev)
            ...
            if (trim(yname) == ppp_tab(j)%name) then
               if (ppp_tab(j)%isint) then
                  ...
                  if (iread == 0) then
                     ...
                     write(nud,'("* ",A," = ",G10.4," *")') yname(1:15),ppp_tab(j)%preal
                  else
                     write(nud,'("* ",A," :",I5," items *")') yname(1:15),iread
                  endif
               endif
      ...
      if (nlat_ppp(1) /= 0 .and. nlat_ppp(1) /= nlat) then
         ...
         write(nud,*) "NLAT in PUMA : ",nlat    ," <",trim(resolution_namelist),">"
         write(nud,*) "Aborting ..."
         stop
      endif
      if (nlev_ppp(1) /= 0 .and. nlev_ppp(1) /= nlev) then
         ...
         write(nud,*) "NLEV in PUMA : ",nlev    ," <",trim(resolution_namelist),">"
         write(nud,*) "Aborting ..."
         stop
      endif
      write(nud,*) "*******************************"
...
!     divergence, vorticity -> u*cos(phi), v*cos(phi)
      ...
      !write(nud,*) "x1",gu2(30), nstep   !HALLOCYCLONE

! ADD HALLO CYCLONE

      !call cyclones2() !output gu,gv is transformed into tendency by calcgp

      !write(nud,*) "x2",gu(30,4), nstep   !HALLOCYCLONE
      ...
      !do jhor =1, nhor
      !  do jlev = 1, NLEV
      !    gu(jhor,jlev)=gu(jhor,jlev)+gu2(jhor) 
      !    gv(jhor,jlev)=gv(jhor,jlev)+gv2(jhor)     
      !  enddo
      !enddo

!         call gridpointd

      !replace cyclone wind!
      !do jhor =1, nhor
      !  jlon = mod(jhor-1,nlon) + 1
      !  jlat = ((jhor - 1) / nlon) + 1
      !  if (asin(gsinalt(jhor)).ge.84/180.*PI) then
      !    do jlev = 1, NLEV
      !      gu(jhor,jlev)=gu2(jhor) 
      !      gv(jhor,jlev)=gv2(jhor) 
      !    enddo
      !  endif
      !enddo

      !write(nud,*) "x3",gu(30,4), nstep   !HALLOCYCLONE
      gut(:,:) = gu(:,:) * gt(:,:)
      gvt(:,:) = gv(:,:) * gt(:,:)
      gke(:,:) = gu(:,:) * gu(:,:) + gv(:,:) * gv(:,:)


      !write(nud,*) "x4",gu(30,4),gut(30,4), nstep   !HALLOCYCLONE
...
!     ===================
      ...
      real ::  disttocc(nhor) !distance to cyclone center in radians (great circle distance)
      real ::  cytanvel(nhor), cytanvelp(nhor)

      integer ::  jlon, jcyn, jcys
      real ::  cn_lat(ncyn),cn_lon(ncyn)
      real ::  cs_lat(ncys),cs_lon(ncys)

      real ::  csinlat,ccoslat
      real ::  cy_angle

      !intermediate output
      real ::  vouta1,vouta2,vouta3
      real ::  voutb1,voutb2,voutb3
      real ::  voutc1,voutc2,voutc3
      real ::  vfin1, vfin2, vfin3
!      real (kind=8) ::  
    ...
    subroutine gridpointd
      ...
      ! dimensionalize
      !dp(:)=psurf*exp(gp(:))
               write(nud,*) "time21",du2(3300),sz(2,2), nstep   !HALLOCYCLONE      
      do jlev = 1 , NLEV
      	do jhor = 1, NHOR      
          du(jhor,jlev)=cv*gu(jhor,jlev)*sqrt(rcsq(jhor))
          dv(jhor,jlev)=cv*gv(jhor,jlev)*sqrt(rcsq(jhor))
!       dt(:,jlev)=ct*(gt(:,jlev)+t0(jlev))
        enddo
      enddo
               write(nud,*) "time22",du2(3300),du(2,2), nstep   !HALLOCYCLONE
!
!*    add tendencies
!
      do jlev = 1 , NLEV
      	do jhor = 1, NHOR
!          dudt(jhor,jlev)=dudt(jhor,jlev)+(du2(jhor)-du(jhor,jlev))/deltsec2
!          dvdt(jhor,jlev)=dvdt(jhor,jlev)+(du2(jhor)-dv(jhor,jlev))/deltsec2
          dudt(jhor,jlev)=dudt(jhor,jlev)+(du2(jhor)-du(jhor,jlev))/deltsec2
          dvdt(jhor,jlev)=dvdt(jhor,jlev)+(dv2(jhor)-dv(jhor,jlev))/deltsec2
          !write(nud,*) "time2211",jhor,du2(jhor),du(jhor,jlev),deltsec2,dvdt(jhor,jlev), nstep   !HALLOCYCLONE
        enddo
      enddo
      !
!     de-dimensionalize
!
               write(nud,*) "time23",du2(2),dv2(2),du(2,2),dv(2,2),dudt(2,2),deltsec2, nstep   !HALLOCYCLONE
      do jlev = 1 , NLEV
      	do jhor = 1, NHOR
          gudt(jhor,jlev)=dudt(jhor,jlev)/sqrt(rcsq(jhor))/cv/ww
          gvdt(jhor,jlev)=dvdt(jhor,jlev)/sqrt(rcsq(jhor))/cv/ww 
!       gudt(jhor,jlev)=dudt(jhor,jlev)/sqrt(rcsq(jhor))/cv/ww                    &
!     &             +gudt(jhor,jlev)
!       gvdt(jhor,jlev)=dvdt(jhor,jlev)/sqrt(rcsq(jhor))/cv/ww                    &
!     &             +gvdt(jhor,jlev)
...
!
               write(nud,*) "time24",du2(3300),gudt(2,2), nstep   !HALLOCYCLONE
...
!      enddo
               write(nud,*) "time25",du2(3300),gudt(2,2), nstep   !HALLOCYCLONE
      call uv2dv(gudt,gvdt,sdf2,szf2)
               write(nud,*) "time26",du2(3300),sdf2(2,2), nstep   !HALLOCYCLONE
!      call mpsumsc(stf,stt,NLEV)
      call mpsumsc(sdf2,sdt2,NLEV)
      call mpsumsc(szf2,szt2,NLEV)
      !PUT sdf into spectral somewhere
               write(nud,*) "time27",du2(3300),sdt2(2,2), nstep   !HALLOCYCLONE
    ...
    return 
      ...
      subroutine cyclones2
      use pumamod

      real ::  gcoslat(nhor),gsinlat(nhor) 
      real ::  gcoslon(nhor) 
      real ::  gsinlon(nhor)
      real ::  lon_rad(nhor)
      real ::  lat_rad(nhor)
...
!      do jhor = 1 , NHOR
!        gcoslat(jhor) = sqrt(1.0-gsinalt(jhor)*gsinalt(jhor)) !gsinalt is sin of lat
!        !gcoslat(jhor) = sqrt(1.0-sidalt(jhor)*sidalt(jhor)) !gsinalt is sin of lat
!        lat_rad(jhor) = asin(gsinalt(jhor))
!      enddo 
      ...
      do jhor = 1 , NHOR
        ...
        sinxxx = gsinalt(jhor)!sid(jlat)
        lat_rad(jhor) = asin(sinxxx)
        gcoslat(jhor) = cos(lat_rad(jhor))
        gsinlat(jhor) = sin(lat_rad(jhor))
        !write(nud,*) jhor,jlon,jlat,gsinalt(jhor)

        lon_rad(jhor) = (jlon -1.0 )/nlon *2.0*PI
        gcoslon(jhor) = cos(lon_rad(jhor))
        gsinlon(jhor) = sin(lon_rad(jhor))
      enddo

      !do jlat =1, nlat
      !  write(nud,*) jlat,sid(jlat),sidalt(jlat)
      !enddo

      !do jhor =1, nhor
      !  write(*,*) jhor,gsinalt(jhor),gsinlat(jhor)
      !enddo
...
!      endif
      ...
      ! north pole central cyclone
      do jhor = 1 , NHOR
        csinlat=1.
        ccoslat=0.
        disttocc(jhor)=rad2meter(distance(csinlat,ccoslat,cn_lon(jcyn),gsinlat(jhor),gcoslat(jhor),lon_rad(jhor)))
!        write(nud,*) jhor,lat_rad(jhor),lon_rad(jhor),distance(csinlat,ccoslat,cn_lon(jcyn),gsinlat(jhor),gcoslat(jhor), &
!   &                     lon_rad(jhor)),disttocc(jhor)!,csinlat,ccoslat,cn_lon(jcyn),gsinlat(jhor),gcoslat(jhor)
        cytanvelp(jhor)=tanvel(disttocc(jhor),cn_rmv,cn_vmax,cn_alpha)
        !write(nud,*) jhor,disttocc(jhor),cn_rmn,cn_vmax,cn_alpha,cytanvelp(jhor)
!          do jlev = 1, NLEV
            du2(jhor)=du2(jhor)+cytanvelp(jhor)*gcoslat(jhor)
!          enddo
!          do jlev = 1, NLEV
!            gu(jhor,jlev)=gu(jhor,jlev)+cytanvelp(jhor)*gcoslat(jhor)/cv 
!          enddo
      enddo
      ...
      do jcyn = 1 , ncyn
          ...
          cytanvel(jhor)=tanvel(disttocc(jhor),cn_rmv,cn_vmax,cn_alpha) ! IF TOO LARGE REMOVE HAVE CUTOFF CONDITION AT CERTAIN DIST

          ! calculate angle between longitude band and radial cyclone vector
          !cy_angle = acos( (cn_lon(jcyn) -  )/() )

          ! transform vector from (0,0,v_t) to spherical
          !transvec(vout1,vout2,vout3,vin1,vin2,vin3)
          !transvec1s3(vout1,vout2,vout3,vin3,lon)

          aalpha = cn_lon(jcyn)
          abeta  = 90-abs(cn_lat(jcyn))


          call transvec1s3(vouta1,vouta2,vouta3,cytanvelp(jhor),lon_rad(jhor)) !start with polar cyclone ! transform to carthesian
          call transvec2(voutb1,voutb2,voutb3,vouta1,vouta2,vouta3,aalpha) ! turn around z axis by angle alpha=cn_lon
          call transvec3(voutc1,voutc2,voutc3,voutb1,voutb2,voutb3,abeta) ! turn around x axis by angle beta=90-abs(cn_lat)
          call transvec4(vfin1,vfin2,vfin3,voutc1,voutc2,voutc3,lat_rad(jhor),lon_rad(jhor)) ! transform back into spherical
          !if (cytanvelp(jhor).ne.0) then
          !  write(nud,*) jhor,cytanvelp(jhor),vfin1,vfin2,vfin3
          !endif
          vlon=vfin3
          vlat=vfin2
...
!             gu(jhor,jlev)=gu(jhor,jlev)+vlon*gcoslat(jhor)/cv 
!             gv(jhor,jlev)=gv(jhor,jlev)+vlat*gsinlat(jhor)/cv ! HOPEFULLY ALL THIS IS IN ALTERNATIVE GRID!!?!?!
!           	if (jlev==4 .and. jhor==3000) write(nud,*) lat_rad(jhor)*180./PI,lon_rad(jhor)*180./PI,jcyn
!           	if (jlev==4 .and. jhor==3000) write(nud,*) gu(jhor,jlev), vlat, vlat*gsinlat(jhor)/cv, nstep   !HALLO            
!          enddo


            du2(jhor)=du2(jhor)+vlon*gcoslat(jhor) 
            dv2(jhor)=dv2(jhor)+vlat*gsinlat(jhor) ! HOPEFULLY ALL THIS IS IN ALTERNATIVE GRID!!?!?!
          	if (jhor==3300) write(nud,*) lat_rad(jhor)*180./PI,lon_rad(jhor)*180./PI,jcyn
          	if (jhor==3300) write(nud,*) du2(jhor), vlat, vlat*gsinlat(jhor)/cv, nstep   !HALLO            
      ...
      subroutine transvec1colat(vout1,vout2,vout3,vin1,vin2,vin3,lon,colat)
      real,intent(out) :: vout1,vout2,vout3
      real,intent(in)  :: vin1,vin2,vin3,lon,colat
      vout1 = sin(colat)*cos(lon)*vin1 + cos(colat)*cos(lon)*vin2 - sin(lon)*vin3
      vout2 = sin(colat)*cos(lon)*vin1 + cos(colat)*sin(lon)*vin2 + cos(lon)*vin3
      vout3 = cos(colat)          *vin1 - sin(colat)          *vin2
      end subroutine

      subroutine transvec1(vout1,vout2,vout3,vin1,vin2,vin3,lon,lat)
      !geographic coord to carthesian
      !S
      real,intent(out) :: vout1,vout2,vout3
      real,intent(in)  :: vin1,vin2,vin3,lon,lat
      vout1 = cos(lat)*cos(lon)*vin1 + sin(lat)*cos(lon)*vin2 - sin(lon)*vin3
      vout2 = cos(lat)*sin(lon)*vin1 + sin(lat)*sin(lon)*vin2 + cos(lon)*vin3
      vout3 = sin(lat)          *vin1 - cos(lat)          *vin2
      end subroutine

      subroutine transvec1s3(vout1,vout2,vout3,vin3,lon)
      !should lon here be something other than sinlon? 
      ! maybe the longitude it would have if it were at the pole with the correlating distance?
      real,intent(out) :: vout1,vout2,vout3
      real,intent(in)  :: vin3,lon
      vout1 = sin(lon)*vin3
      vout2 = cos(lon)*vin3
      vout3 = 0
      end subroutine

      subroutine transvec2(vout1,vout2,vout3,vin1,vin2,vin3,alpha)
      !Mz
      real,intent(out) :: vout1,vout2,vout3
      real,intent(in)  :: vin1,vin2,vin3,alpha
      vout1 = cos(alpha)*vin1 + sin(alpha)*vin2
      vout2 =-sin(alpha)*vin1 + cos(alpha)*vin2
      vout3 =                                    vin3
      end subroutine

      subroutine transvec3(vout1,vout2,vout3,vin1,vin2,vin3,beta)
      !Mx
      real,intent(out) :: vout1,vout2,vout3
      real,intent(in)  :: vin1,vin2,vin3,beta
      vout1 = vin1 
      vout2 =       cos(beta)*vin2 + sin(beta)*vin3
      vout3 =     - sin(beta)*vin2 + cos(beta)*vin3
      end subroutine

      subroutine transvec4(vout1,vout2,vout3,vin1,vin2,vin3,lat,lon)
      !ST
      real,intent(out) :: vout1,vout2,vout3
      real,intent(in)  :: vin1,vin2,vin3,lon,lat
      vout1 = cos(lat)*cos(lon)*vin1 + cos(lat)*sin(lon)*vin2 + sin(lat)*vin3
      vout2 = sin(lat)*cos(lon)*vin1 + sin(lat)*sin(lon)*vin2 - cos(lat)*vin3
      vout3 = - sin(lon)        *vin1 + cos(lon)          *vin2
      end subroutine
      ...
      real :: dist                ! in meters 
      real :: rmv                 ! radius of maximum wind (meters)
      real :: vmax                ! maximum velocity (m/s)
      real :: calpha              ! decrease factor
      ...
      real :: czinp               ! height (pressure coord)
      real :: alpha               ! decrease factor
      real :: czlen               ! vertical scale of cyclone
      real :: czmax               ! maximum height of cyclone
...
!     11. Coupling for synchronization runs
      ...
      if (mrnum == 2 .and. nsync > 0) then
         call mrdiff(st,std,NESP*NLEV)
         call mrdiff(sd,sdd,NESP*NLEV)
         call mrdiff(sz,szd,NESP*NLEV)
         call mrdiff(sp,spd,NESP     )
         st(:,:) = st(:,:) + reveps * std(:,:)
         sd(:,:) = sd(:,:) + reveps * sdd(:,:)
         sz(:,:) = sz(:,:) + reveps * szd(:,:)
         sp(:  ) = sp(:  ) + reveps * spd(:  )
