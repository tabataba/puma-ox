...
void ReadImage(struct MapImageStruct *ei, char *filename)
{
   ...
   ch = fgetc(fp);
   if (ch != 'M') return;
   n = fread(&FileSize,sizeof(FileSize),1,fp);
   n = fread(&Reserved,sizeof(Reserved),1,fp);
   n = fread(&OffBits,sizeof(OffBits),1,fp);
   ...
   n = fread(&ImageBMI,sizeof(ImageBMI),1,fp);
...
void TestWindow(int xx, int yy, int wi, int he,
                  int *x, int *y, int *X, int *Y, int *W, int *H)
{
   ...
   XGetGeometry(display,TW,&Parent,x,y,W,H,&border,&depth);
   ...
   if (Debug)
   {
      printf("TW[%x][%x]: %4d / %4d   %4d x %4d x %2d [%2d:%2d]\n",
              (int)TW,(int)Parent,*x,*y,*W,*H,depth,*X,*Y);
   }
}
...
void CreateControlWindow(void)
{
   ...
   if (Model == 0) // PUMA
   {
      if (MRpid < 0) 
         ...
         sprintf(Title1,"Run %2d: PUMA - KlimaCampus Hamburg",MRpid);
   }
   else if (Model == 1) sprintf(Title1,"SAM - KlimaCampus");
   else if (Model == 2) sprintf(Title1,"Planet Simulator (%s) - KlimaCampus",
                     PlanetName);
...
void initgui_(int *model, int *debug, int *lats, int *mrpid, int *mrnum)
{
   ...
   ReadConfig("GUI.cfg");            // Default cfg
   ReadConfig("GUI_last_used.cfg");
