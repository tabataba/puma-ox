...
if len(sys.argv) == 1:
  ...
  ncfile = '/media/Seagate5TB/puma'+'/pumagt_arcb_parameters/rev53_r0.125_res64_radius1.00_taufr1.0_psurf1.0_pref1.0_taus2.00_tausurf360_nmu0/PUMAG.025.nc'
  afac=1
  pfac=1
  outname='test'
#elif len(sys.argv) == 3: 
...
fig, axarr = plt.subplots(1, 1, sharex='col', figsize=(5,4))
#for ax,title,i in zip(axarr.ravel(),titles2,[0,1,2,3]):
#    print ax,title,i
lev = np.array([1000,900,800,700,600,500,400,300,200,100])
ax=axarr
neq=int(nlat/2)
uplot = np.mean(umtmlon[:,neq-3:neq+3],axis=1)
ax.plot(uplot[:],lev[:])
#ax.plot(10/lev[:],lev[:])
#ax.plot(100/lev[:],lev[:])
#ax.plot(2/lev[:],lev[:])
#ax.plot(0.5/lev[:],lev[:])
#ax.plot(0.05/lev[:],lev[:])
ax.plot(10000/lev[:],lev[:])
ax.plot(20000/lev[:],lev[:])
#ax.plot(50000/lev[:],lev[:])
#ax.plot(100000/lev[:],lev[:])


ax.invert_yaxis()
#plt.gca().invert_yaxis()
ax.set_xlabel('equatorial zonal wind (m/s)')
ax.set_ylabel('Pressure (hPa)')
plt.tight_layout()
plt.savefig(outdir+'1'+outname+'.pdf',format='pdf')
print lev
sys.exit(1)
