...
#  ncfile = sys.argv[1]
  ...
  print facs
  it=int(sys.argv[3])
  outname = 'snap'+sys.argv[1]+'_'+sys.argv[3]
...
#for it in range(0,360,5):
if True:
  it1=it
  it2=it+5
  
  tmt = np.mean(t[it1:it2,:,:,:], axis=0)
...
#strm=strm*1E-6
  ...
  uprime = u[it1:it2,:,:,:] - np.reshape(np.mean(u[it1:it2,:,:,:],axis=3),(5,nlev,nlat,1))
  vprime = v[it1:it2,:,:,:] - np.reshape(np.mean(v[it1:it2,:,:,:],axis=3),(5,nlev,nlat,1))
...
#plt.gca().invert_yaxis()
  ...
  titles=['(a) mass streamfunction/zonal mean zonal wind','(b) zonal mean temperature']
