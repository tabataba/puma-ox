...
for i1,irot in enumerate([0.0625, 0.125]):
  for i2,itauf in enumerate([0.1, 1.0, 10.0]):#, 1.0, 10.0]):
    for i3,ips in enumerate([0.2,1.0, 5.0]):#,1.0, 5.0]): #[0.2, 1.0, 5.0]:
      for i4,itausw in enumerate([10]): #[2 ,10]:
        for i5,itausurf in enumerate([3.6, 36 ,360]):
          ...
          if data[mask0,10] > -100:
            ...
            yy=data[mask0,imax]
            xx=vph-data[mask1,i10]
            plt.plot(xx,yy,marker=shapes[i2],color=colors[i3],markersize=sizes[i1])
...
#plt.plot(x11[:,0],3.14*x11[:,0]+10,'.')
plt.ylabel(r'$U$')
plt.xlabel(r'$c-U_{mean}$')
plt.savefig('supcomp_imax'+str(imax)+'_imean'+str(i10)+'.png', dpi=200)
#plt.show()
