...
Tdat=[0,14.5,0,4.4,0,0.42,-0.42,0.89]

# Ro, Ek, At, G, alpha
Earth= [0.02, 0.08, 400, 0.2, 0.04] #moved all planets  scale for G is from 1 to -1 so our scale here goes from 0 to 2 
Mars = [0.1 , 0.04,  12, 0.5, 20] # Mars G moved to fit better with new definition of G
Venus= [300 , 6   , 190, 0.4, 0]
Titan= [ 10 , 0.4 , 5000, 1.5, 10]
  ...
  for ilor,title in enumerate(titles):
    for imu in [1,0]:
      ...
      #for taufac in [1,]:
        ...
        spE= ax.scatter(Earth[0]*(1+2*Exx/16),Earth[2]*(1+2*Eyy/2),c=Edat[ilor],norm=norm,cmap=cmap)
        ax.annotate(r"Earth", xy=(Earth[0],Earth[2]), xytext=(Earth[0],Earth[2]*pos*2),arrowprops=dict(arrowstyle="->"))
        ...
        Exx=Mars[4]
        Eyy=Mars[3]
        spE= ax.scatter(Esx*(1+2*Exx/20.8),Esy*(1+2*Eyy/2),c=Mdat[ilor],norm=norm,cmap=cmap)
        ...
        Exx=Venus[4]
        Eyy=Venus[3]
        spE= ax.scatter(Esx*(1+2*Exx/16),Esy*(1+2*Eyy/2),c=-0.5,norm=normg,cmap=cmapg)
        ...
        if Tdat[ilor] != 0: 
            cc=Tdat[ilor]
            normcc=norm
        spE= ax.scatter(Esx*(1+2*Exx/16),Esy*(1+2*Eyy/2),c=Tdat[ilor],norm=norm,cmap=cmapg)
...
##*(1+0.19*2.05*Earth[4]/10),Earth[2]*(1+0.28*1.6*Earth[3]),c=1,cmap=cmapg)
        ...
        plt.tight_layout()
        #plt.show()
        plt.savefig(outdirpng+'1lorP22_'+title+t2[isd]+'_RoAt_tauf'+str(taufac)+'_nmu_'+str(imu)+'.png',format='png',dpi=200)
