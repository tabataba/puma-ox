...
#with open('max_file.txt','r') as f_handle:
maxsav=np.genfromtxt('max_file.txt')

maxx=np.max(maxsav,axis=0)
minx=np.min(maxsav,axis=0)
tmin = minx[0]
tmax = maxx[1]
stmax= maxx[2]
umax = maxx[3]
...
aaa=2.0/12.0
emflevels=np.around(np.arange(-1,1.01,aaa)*emax,2)



#aaa=2.0/18.0#plt.text(0, 0, ncfile, fontsize=8)
...
plt.tight_layout()
plt.savefig(outdir+'2'+outname+'_'+str(args.it)+'.pdf',format='pdf')
...
#plt.gca().invert_yaxis()
plt.tight_layout()
plt.savefig(outdir+'2x'+outname+'_'+str(args.it)+'.pdf',format='pdf')
...
#plt.gca().invert_yaxis()
plt.tight_layout()
plt.savefig(outdir+'2y'+outname+'_'+str(args.it)+'.pdf',format='pdf')
...
plt.tight_layout()
plt.savefig(outdir+'4'+outname+'_'+str(args.it)+'.pdf',format='pdf')
#here
...
plt.tight_layout()
plt.savefig(outdir+'4x'+outname+'_'+str(args.it)+'.pdf',format='pdf')
