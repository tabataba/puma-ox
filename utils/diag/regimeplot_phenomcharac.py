  ...
  #for ilor,title in enumerate(titles):
    for imu in [1]:
      ...
      for taufac in [1]:
        ...
        mask_eqx=mask2&(data2[:,9]>0.25)&(data2[:,9]<1)&(data2[:,10]<10)
        ...
        print E_rel.shape
        #cmap=m
        sp = ax.scatter(Ro[mask],At[mask],c=E_rel[mask],cmap=cmap,norm=normx,s=60*2.0)
        #sp = ax.scatter(Ro2[mask_eq],At2[mask_eq],s=60*2,marker='s')
        sp1 = ax.scatter(Ro2[mask_eq],At2[mask_eq],s=50*2,marker='x',c='k')
        sp1x= ax.scatter(Ro2[mask_eqx],At2[mask_eqx],s=50*2,marker='x',c='g')
        sp2 = ax.scatter(Ro2[mask_eq2],At2[mask_eq2],s=60*2,marker='+',c='k')
        ...
        ax.annotate("", xy=(asx+aqx,asy), xytext=(asx, asy),arrowprops=dict(arrowstyle="->"))
        ax.annotate("", xy=(asx,asy+aqy), xytext=(asx, asy),arrowprops=dict(arrowstyle="->"))
        ax.annotate(r"$\alpha_S$", xy=(asx+aqx,asy), xytext=(asx+(aqx)*0.85, asy))
        ax.annotate(r"$\mathcal{G}$", xy=(asx,asy+aqy), xytext=(asx, asy+aqy))
...
##*(1+0.19*2.05*Earth[4]/10),Earth[2]*(1+0.28*1.6*Earth[3]),c=1,cmap=cmapg)
        ...
        plt.tight_layout()
        #plt.show()
        plt.savefig(outdirpng+'1phenom_En_RoAt_tauf'+str(taufac)+'_nmu_'+str(imu)+'.png',format='png',dpi=200)
