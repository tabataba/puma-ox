...
if args.path == None and args.folder == None: 
  path = 'data/lorenz_en/rev53_r0.125_res64_radius1.00_taufr1.0_psurf1.0_pref1.0_taus10.00_tausurf3.6_nmu0/'
else:
...
for filen in onlyfiles:
  if "_dm" not in filen:
    ...
    #shape = dat.shape
    #print dat.shape[0]
    if iii==0: datsav = np.empty((12,dat.shape[0],dat.shape[1]))
    #print datsav.shape
...
#    datsav[iii,:,:] = dat
  ...
  if "_dm" in filen:
    ...
    #shape = dat.shape
    #print dat.shape[0]
    if jjj==0: dmsav = np.empty((12,dat.shape[0],dat.shape[1]))
    #print datsav.shape
    #print np.mean(dat,axis=1)
    dmsav[jjj,:,:] = dat
    #if iii>0:  prev = np.copy(tot)
...
print 'month'
print np.mean(datsav,axis=2)

print 'year'
print np.mean(np.mean(datsav,axis=2),axis=0)

print 'year_dm'
print np.mean(np.mean(dmsav,axis=2),axis=0)

print 'year_diurnal'
print np.mean(np.mean(datsav-dmsav,axis=2),axis=0)
