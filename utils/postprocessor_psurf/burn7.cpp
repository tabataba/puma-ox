...
#define V0 "burn 7.1 (23-Feb-2011)"
...
#ifdef NETCDF_OUTPUT
#define TITLE        "PUMA DATA"
#define HISTORY      "Created by PumaBurner 6.1"
...
void NetOpen(char *NetFileName)
{
   ...
   if (VerType == 's') // sigma
   {
      for (jlev = 0 ; jlev < OutLevs ; ++jlev) Outlev[jlev] = mol[jlev];
   }
...
inline int ReadINTRecord(void)
{
   ...
   e = ReadFCW();
   if (b != e) Abort("record control word mismatch in ReadINTRecord");
   if (Endian) SwapIEEE((char *)&k);
   return k;
}
...
inline int Skip_FORTRAN_Record(void)
{
    ...
    fcwe = ReadFCW();
    if (fcws != fcwe) Abort("record control word mismatch in Skip_FORTRAN_Record");
    return fcws;
}
...
inline int Read_FORTRAN_Record(void)
{
    ...
    fcwe = ReadFCW();
    if (fcws != fcwe) Abort("record control word mismatch in Read_FORTRAN_Record");
...
inline int Read_FORTRAN_Double_Record(void)
{
    ...
    fread(Record_char,1,fcws,fpi);
    fcwe = ReadFCW();
    if (fcws != fcwe) Abort("record control word mismatch in Read_FORTRAN_Double_Record");
...
int ReadHeaderRecord(void)
{
   ...
   if (fcwe != fcws)
   {
      printf("FCW before header = %d\n",fcws);
      printf("FCW after  header = %d\n",fcwe);
      printf("Expected          = %d\n",HeadSize);
      printf("The FORTRAN control words (FCW) before and after the header\n");
      printf("record do not match. One possible cause is an incomplete\n");
      printf("data file due to a model crash\n");
      Abort("FCW mismatch in ReadHeaderRecord");
   }
   return 0;
}
...
void PumaProcess(void)
{
   ...
   if (net_heat->needed)
   {
      ...
      + All[176].hgp + All[177].hgp + All[146].hgp + All[147].hgp - All[220].hgp;
   }
   ...
   for (code = 0; code < CODES; code++)
   if (All[code].needed && All[code].pgp.size() && !All[code].pfc.size())
   {
      All[code].SetPFour();
...
void PostProcess(void)
{
   ...
   if (EndOfMonth)
   {
      sprintf(tb,"Processed Month %2d   Year %4d", OldDate.tm_mon,OldDate.tm_year);
      l = strlen(tb);
      if (MeanCount > 1)
      {
...
void PumaControl(void)
{
   ...
   while (1)
   {
         ...
         if (HeadIn[2] != HeadSt[2] || HeadIn[3] != HeadSt[3])
	 {
      ...
      if ((HeadIn[2] / 100 % 100) > LastMonth) // Ignore rest of file
      {
         Eof = 1;
         if (Multi)
         {
      ...
      if (Eof) // Process last read term and finish
      {
         ...
         if (Mean)
         {
            HeadOu[2] = NewDate.tm_year * 10000 + NewDate.tm_mon * 100;
            HeadOu[3] = 0;
         }
         else
         {
            HeadOu[2] = NewDate.tm_year * 10000 + NewDate.tm_mon * 100
                      + NewDate.tm_mday;
            HeadOu[3] = NewDate.tm_hour * 100 + NewDate.tm_min;
         }
         PumaProcess();
         PostProcess();
         Dependencies();
      ...
      if (NewDate.tm_mon < FirstMonth) /* Skip months before FirstMonth */
      {
         SkipPumaArray();
         if (Debug)
         {
      ...
      if (OldDate.tm_mon > 0)
      {
         ...
         EndOfMonth = NewDate.tm_mon != OldDate.tm_mon;

         if (memcmp(&NewDate,&OldDate,sizeof(struct tm)))
         {
            PumaProcess();
            PostProcess();
            Dependencies();
...
void InitAll(void)
{
   ...
   All[129].Init("sg"   ,"geopotential"                    ,"m2 s-2"   ,1);
...
void parini(void)
{
   ...
   if (VerType == 's')   /* model levels */
   {
      ...
      for (i=0 ; i < OutLevs ; ++i)
      {
         ...
         sprintf(tb,"Level %4d = %10d",i+1,mol[i]);
         LeftText(tb);
      }
   }
...
void AnalyzeFile(void)
{
   ...
   if (fcb == 8)
   {
      ...
      fce = ReadFCW();
      if (fce != fcb) Abort("Wrong FORTRAN control word after PUMA header");
      ...
      fce = ReadFCW();
      if (fce != fcb) Abort("FCW mismatch on level record");
...
void WriteGradsCtl()
{
   ...
   if (HorType == 'z')
      ...
      fprintf(fp,"XDEF %5d LINEAR 0.0 %10.4f\n",Gons,(360.0/Gons));
   ...
   if (OutLevs < 2)
   {
      if (VerType == 'p') fprintf(fp,"ZDEF 1 LINEAR %10.4f 1\n",hPa[0]);
      ...
      if (VerType == 'p')
      {
         ...
         for (j=0 ; j < OutLevs ; j+=8)
         {
            ...
            if (HorType == 'z' && hPa[0] < hPa[OutLevs-1])
               fprintf(fp,"%10.4f",hPa[OutLevs-1-i]);
            else
               fprintf(fp,"%10.4f",hPa[i]);
            fprintf(fp,"\n");
         }
         if (HorType == 'z' && hPa[0] < hPa[OutLevs-1])
