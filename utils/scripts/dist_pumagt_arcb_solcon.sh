...
DIRNAME="pumagt_arcb_parameters_solcon2/"
...
preffac="$(round 1 1)"
taus="$(round 0.0 2)"
for i in 0.125 #2.0 0.0625 0.125 0.25 0.5 1.0 #2.0 4.0 8.0   ###1.0 0.44 0.14 0.044 2.0 #0.44 1.0
do
for k in 360 #3.6 36 360 #0.5 7 30 90 360 
do
for iii in 1 0 #1
do
for taus in 0.20 2.00 10.00 #0.00 2.00 10.00
do
for sol in 0.125 0.25 0.5 2.0 4.0 8.0 #1
do
...
#do
  ...
  xx=03
  RUNNAME=$LOCATION/$BASENAME$i$NAME2$j$NAME21$a$NAME22$fricfac$NAME23$psurffac$NAME24$preffac$NAME25$taus$NAME3$k$NAME4$iii"solcon"$sol
  ...
  sed -i -e 's/real    :: obliq   =  OBLIQ_EARTH/real    :: obliq   =  0.00/g' radmod.f90
