...
DIRNAME="pumagt_arcb_rot_newsurf_rev58_v1.4_taus2"
HOM="/home/pam/ftabatabavakili/"
LOCATION=$DAT$DIRNAME
BASENAME="rot_r"
NAME2="_res"
NAME3="_taus02_tausurf"
NAME4="_nmu"
...
for i in 0.125 #0.25 0.5 0.0625 1.0 #0.03125  0.0455  #0.0625 #2.0 4.0 8.0 0.125 0.25 0.5 1.0 #2.0 4.0 8.0 #2.0 4.0 8.0 #0.125 0.25 0.5 1.0 #2.0 4.0 8.0   ###1.0 0.44 0.14 0.044 2.0 #0.44 1.0
do
for k in 360 0.5 #7 30 90 360   #0.5 7 30 90 360 
do
for iii in 0 1 
do
...
#do
  ...
  if [ $j == 64 ]; then xx=7; zz=240; fi
  if [ $j == 128 ]; then xx=24; zz=240; fi
  if [ $j == 256 ]; then xx=24; zz=480; fi
  if [ $j == 512 ]; then xx=120; zz=720; fi
  cp -rf $HOM/puma/trunk/pumagt $LOCATION/$BASENAME$i$NAME2$j$NAME3$k$NAME4$iii
  cd $LOCATION/$BASENAME$i$NAME2$j$NAME3$k$NAME4$iii
