...
DIRNAME="pumagt_arcb_parameters_noseasons/"
...
do
for psurffac in 1.0 #0.01 0.1 1.0 10.0 100.0 #0.02 30.0
do
...
for iii in 1 0 #1
do
for taus in 0.20 #10.00 #0.20 2.00 #10.00 #0.20 #0.00 2.00 10.00
do
...
#do
  ...
  sed -i -e 's/mpiexec puma.x 64 10 $NPROC/mpiexec puma.x '$j' 20 $NPROC/g' most_puma_run_mpi
 ...
 # cp -rf start.q_bkp start.q
  ...
  cd ..
  echo $RUNNAME2 >> brandnew_runs2.txt
  #cd ..
done
done
done
