...
DAT="/data/pam/ftabatabavakili/" #"/network/group/aopp/planetary/PLR005_TABATABAVAKILI_PUMAGT/" #"/data/pam/ftabatabavakili/"
...
#do
for fricfac in 0.1 1.0 10.0
do
for psurffac in 0.2 1.0 5.0
do
for i in 0.0625 0.125 0.25 0.5 1.0 #2.0 MAYBE LATER  #2.0 4.0 8.0   ###1.0 0.44 0.14 0.044 2.0 #0.44 1.0
do
for k in 3.6 #36 #3.6 36 360 #0.5 7 30 90 360 #FIRST 360 REST MAYBE LATER!!!
do
for iii in 1 0 #1
do
for taus in 0.00 2.00 10.00
...
#do
  ...
  #qq=$(squeue -u ftabatabavakili) 
  qqwc=$(squeue -u ftabatabavakili | wc -l)
  echo $qqwc
  if [ $qqwc -gt 81 ]; then echo "TOO MANY"; exit 1; fi 
  preffac=$psurffac
  ...
  if [ $fricfac == 0.1 ]; then xx=24; zz=1440; fi
  if [ $psurffac == 0.2 ]; then xx=24; zz=1440; fi
  ...
  #echo $RUNNAME
  if [ ! -d $RUNNAME ]; then 
    echo $RUNNAME' missing'; 
    echo $RUNNAME2 >> missing.txt;  
    let "xxxxx+=1" ; 
    #echo test
    #exit 1
 ...
 # cp -rf start.q_bkp start.q
  ...
  cd ..
  echo $RUNNAME2 >> missing_new1.txt
