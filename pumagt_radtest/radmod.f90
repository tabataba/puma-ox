...
!      real    ::  obliq    = OBLIQ_EARTH   ! Obliquity [deg] (AMIP-II)
      ...
      integer  :: nmumean = 0 ! daily mean of solar zenith angle (on/off 1/0)
...
!     ====================
      ...
      if (nmumean > 0) then
        ...
        declmean_rad = -obliq_rad*cos(TWOPI*time_days / real(dpy))
        do jhor = 1 , NHOR
...
!     dimensional arrays
      ...
      real :: deltsec
      real :: tausurfx = 15552000 !0.5 years 
...
!     SW transfer
!     ===========
      Dradis(:,1) = gsol0 * gmutt(:) ! gsol0 * (1-albbnd) * gmutt(:) 
!gmu_t(:)!* gmu0(:)
...
!      Uradis(:,nlep) = Dradis(:,nlep) * albsfc
      Uradis(:,nlep) = Dradis(:,nlep) *albbnd !* 0.0
      do jlev = nlev, 1, -1
...
! add that convection happens within timeframe of 2 hours not instantly
       gtt(jhor,jlev) = gtt(jhor,jlev) + zdtdt(jlev,jhor)/(ct*ww)*deltsec/7200.0
      enddo
