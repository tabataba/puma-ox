...
real,allocatable :: sconv(:)
real,allocatable :: srad(:)
real,allocatable :: saconv(:)
real,allocatable :: dswtoa(:)
real,allocatable :: ulwtoa(:)
...
allocate(gsin(nhor))      ; gsin(:)  = 0.0 ! sin(phi)
...
call mpscdn(sidalt,NHPP) 
call mpscdn(gwd,NHPP) ! real (kind=8)
call mpscrn(csq,NLPP)
do jlat = 1, nlat/2
   sidalt(2*jlat-1) = sid(jlat)
   sidalt(2*jlat) = sid(nlat-jlat+1)
enddo

do jlat = 1 , NLPP
   rcsq(1+(jlat-1)*NLON:jlat*NLON) = 1.0 / csq(jlat)
   gsin(1+(jlat-1)*NLON:jlat*NLON) = sid(jlat) 
