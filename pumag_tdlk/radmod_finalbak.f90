...
!     ===========
	...
	bt(:) = zdgsch(:,jlev)
        ...
        do jhor = 1, nhor
         if(abs((ztaul(jhor,jlev)/2)-diffcoef(jhor,jlev)*Log(bb(jhor)/bl(jhor))).gt.1.E-10) then
         ED1(jhor) = ((bl(jhor) - bb(jhor)*exp(-(ztaul(jhor,jlev)/2.)/diffcoef(jhor,jlev)))* &
                 (ztaul(jhor,jlev)/2.))/(((ztaul(jhor,jlev)/2.)-diffcoef(jhor,jlev)*Log(bb(jhor)/bl(jhor))))
         else
         ED1(jhor) = ((bl(jhor) - bb(jhor)*exp(-(ztaul(jhor,jlev)/2.)/diffcoef(jhor,jlev)))* &
        ...
        do jhor = 1, nhor
         if(abs((ztaul(jhor,jlev)/2)-diffcoef(jhor,jlev)*Log(bl(jhor)/bt(jhor))).gt.1.E-10) then
         ED2(jhor) = ((bt(jhor) - bl(jhor)*exp(-(ztaul(jhor,jlev)/2.)/diffcoef(jhor,jlev)))* &
                 (ztaul(jhor,jlev)/2.))/(((ztaul(jhor,jlev)/2.)-diffcoef(jhor,jlev)*Log(bl(jhor)/bt(jhor))))
         else
         ED2(jhor) = ((bt(jhor) - bl(jhor)*exp(-(ztaul(jhor,jlev)/2.)/diffcoef(jhor,jlev)))* &
                 (ztaul(jhor,jlev)/2.))/epsilon_r
         endif
	...
	bl(:) = zdgsc(:,jlev)
        ...
        do jhor = 1, nhor
         if(abs((ztaul(jhor,jlev)/2)-diffcoef(jhor,jlev)*Log(bl(jhor)/bt(jhor))).gt.1.E-10) then
         EU1(jhor) = ((bt(jhor) - bl(jhor)*exp(-(ztaul(jhor,jlev)/2.)/diffcoef(jhor,jlev)))* &
                 (ztaul(jhor,jlev)/2.))/(((ztaul(jhor,jlev)/2.)-diffcoef(jhor,jlev)*Log(bl(jhor)/bt(jhor))))
         else
         EU1(jhor) = ((bt(jhor) - bl(jhor)*exp(-(ztaul(jhor,jlev)/2.)/diffcoef(jhor,jlev)))* &
                 (ztaul(jhor,jlev)/2.))/epsilon_r
         ...
         if(abs((ztaul(jhor,jlev)/2)-diffcoef(jhor,jlev)*Log(bb(jhor)/bl(jhor))).gt.1.E-10) then
         EU2(jhor) = ((bl(jhor) - bb(jhor)*exp(-(ztaul(jhor,jlev)/2.)/diffcoef(jhor,jlev)))* &
                 (ztaul(jhor,jlev)/2.))/(((ztaul(jhor,jlev)/2.)-diffcoef(jhor,jlev)*Log(bb(jhor)/bl(jhor))))
         else
...
!     Exchange between the surface and the bottom layer atm.
	  ...
	  A(1) = -gascon/akap*delta_p/ga*zt(jhor,nlev) &
 ...
 1000 continue
       ...
       do jlev=2,NLEM
        jlep=jlev+1
        if(zth(jhor,jlev) < zth(jhor,jlep)) then
         ibase=jlep
         itop=jlev-1
         zsum1=zskap(jlep)*dsigma(jlep)+zskap(jlev)*dsigma(jlev)
         zsum2=zskap(jlep)*dsigma(jlep)*zth(jhor,jlep)                  &
     &           +zskap(jlev)*dsigma(jlev)*zth(jhor,jlev)
         ztht=zsum2/zsum1
         icon=1
        endif
       enddo
       if(icon > 0) then
        do jlev=nlev,1,-1
          if(itop == jlev .and. zth(jhor,jlev) < ztht) then
          itop=jlev-1
          zsum1=zsum1+zskap(jlev)*dsigma(jlev)
          zsum2=zsum2+zskap(jlev)*dsigma(jlev)*zth(jhor,jlev)
          ztht=zsum2/zsum1
          endif
        enddo
        do jlev=1,NLEV
...
!            write(*,*) 'too many iterations'
        ...
        if(icon.eq.0 .and. dgtsurf(jhor).le.zth(jhor,nlev)) then
          goto 1001
        endif
 1001 continue
...
!
      do jlev=1,NLEV
       zdtdt(jhor,jlev)=(zth(jhor,jlev)*zskap(jlev)-zt(jhor,jlev))/deltsec2
!       dtdt(:,jlev)=dtdt(:,jlev)+zdtdt(:,jlev)
!added by Yixiong and Salvatore (Note that the time difference)
          qconv(jhor,jlev)=(gascon/akap*(dsigma(jlev)*zdgp(jhor))/ga*zdtdt(jhor,jlev))!/deltsec2
          qconv_int(jhor)=qconv_int(jhor)+qconv(jhor,jlev)
          saconv(jhor)=saconv(jhor)+qconv(jhor,jlev)/(zth(jhor,jlev)*zskap(jlev))
!          sconv(jhor)=sconv(jhor)-qconv(jhor,jlev)/dgtsurf(jhor)       
       gtt(jhor,jlev) = gtt(jhor,jlev) + zdtdt(jhor,jlev)/(ct*ww)
      enddo
