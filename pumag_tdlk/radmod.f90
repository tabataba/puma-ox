...
!     ====================
      ...
      do jhor = 1,nhor,nlon
        do i = 1, nlon
          gcoslon(jhor+i-1) = cos((i-1)*2*PI/nlon-270.0/360.0*2*PI)      
          if (gcoslon(jhor+i-1).le.0.0) gcoslon(jhor+i-1)=0.0
        enddo
      enddo
...
!     real  arrays 
      real(kind=8) ::  ztaus(nhor,nlev)     ! shortwave optical thickness
      real(kind=8) ::  ztaul(nhor,nlep)     ! longwave optical thickness


!     dimensional arrays
      real(kind=8) ::  zdgp(nhor)           ! surface pressure [Pa]
      real(kind=8) ::  zzdgp(nugp)
      real(kind=8) ::  zdgpsig(nhor,nlev)   ! pressure on sigma levels [Pa]
      real(kind=8) ::  zdgpsigh(nhor,nlep)  ! pressre on simga half levels [Pa]
      real(kind=8) ::  zdgt(nhor,nlev)      ! temperature on sigma levels [K] 
      real(kind=8) ::  zzdgt(nugp,nlev)
      real(kind=8) ::  zdgth(nhor,nlep)     ! temperature on sigma half-levels [K]
      real(kind=8) ::  zdgsc(nhor,nlep)     ! LW source function on sigma levels [W m^-2]
      real(kind=8) ::  zdgsch(nhor,0:nlep)  ! LW source function on sigma half-levels [W m^-2]
      real(kind=8) ::  zdgscg(nhor)         ! LW source function for ground [W m^-2]
      real(kind=8) ::  Uradis(nhor,nlep)    ! upward irradiance on sigma half-levels for SW [W m^-2]
      real(kind=8) ::  Dradis(nhor,nlep)    ! downward irradiance on sigma half-levels for SW [W m^-2]
      real(kind=8) ::  fnets(nhor,nlep)     ! net downward irradiance on sigma half-levels for SW [W m^-2]
      real(kind=8) ::  zhtrs(nhor,nlev)     ! SW heating rate [K s^-1]
      real(kind=8) ::  Uradil(nhor,0:nlep)  ! upward irradiance on sigma half-levels for LW [W m^-2]
      real(kind=8) ::  Dradil(nhor,0:nlep)  ! downward irradiance on sigma half-levels for LW [W m^-2]
      real(kind=8) ::  EU0(nhor), EU1(nhor), EU2(nhor)
      real(kind=8) ::  ED0(nhor), ED1(nhor), ED2(nhor)
      real(kind=8) ::  bb(nhor), bt(nhor), bl(nhor)
      real(kind=8) ::  fnetl(nhor,nlep)     ! net downward irradiance on sigma half-levels for LW [W m^-2]
      real(kind=8) ::  zhtrl(nhor,nlev)     ! LW heating rate [K s^-1]
      real(kind=8) ::  diffcoef(nhor,nlep)  ! coefficient for diffuse approximation

      real(kind=8) ::  zfgtt(nugp,nlev)
      real(kind=8) ::  zdgtsurf(nugp)
      real(kind=8) ::  epsilon_r = 1.E-5
      real(kind=8) ::  lwcnst1(nhor),lwcnst2(nhor)
      real(kind=8) ::  lwflag1(nhor),lwflag2(nhor),lwflag3(nhor),lwflag4(nhor)
