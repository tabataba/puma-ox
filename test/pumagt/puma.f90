...
parameter(SBC          = 5.6704E-8)   ! Stefan-Boltzmann constant [W m^-2 K^-4]
...
parameter(PREF_EARTH   = 100000.0)    ! Reference pressure [Pa] on Earth
...
parameter(SOLCON_EARTH = 1365.0)      ! Solar constant [w/m^2]
parameter(TSOL0_SUN    = 5777.0)      ! Effecive solar temperature
parameter(DSOLP0_EARTH = 1.496E11)    ! Mean distance between Sun and 
!                                     ! Earth [m]
parameter(RSOL0_SUN    = 6.96E8)      ! Mean radius of the Sun [m]
parameter(ALBBND_EARTH = 0.3)         ! Bond albedo  
parameter(ALBSFC_EARTH = 0.12)        ! Surface shortwave albedo  
parameter(EMISFC_EARTH = 1.0)         ! Surface longwave emissiviy
...
integer :: ntrad    =  0 ! interval for running radstep
integer :: ntconv   =  0 ! interval for running dcastep 
...
!                             heating timescale in PUMA (H&Z(94)),
!                             irrelevant for PumaPreProcessor (ppp)
integer  :: nxgp    = 0 ! 0/1 read external gridpoint fields for newtonian 
!                       ! cooling to 1st set of fields (e.g. radiation) 
integer  :: nxcgp   = 0 ! 0/1 read external gridpoint fields for newtonian
!                       ! cooling to 2nd set of field (e.g. convection) 
integer  :: nrad    = 0 ! 0/1 switch for radiative heating 
integer  :: nconv   = 0 ! 0/1 switch for convective adjustment 
integer  :: nheats  = 1 ! 0/1 switch for spectral heating 
integer  :: nheatgp = 0 ! 0/1 switch for gridpoint heating 
...
real, allocatable :: gtt(:,:)    ! temperature tendency
real, allocatable :: radtt(:,:)  ! temperature tendency by radiative heating
...
real, allocatable :: gtdamp(:,:) ! 3D reciprocal damping times
...
real, allocatable :: gmu_diag(:)


!added by Yixiong and Salvatore
real,allocatable :: qconv_int(:)
real,allocatable :: qconv_surf(:)
real,allocatable :: qrad_int(:)
real,allocatable :: sconv_surf(:)
real,allocatable :: srad(:)
real,allocatable :: saconv(:)
real,allocatable :: dswtoa(:)  ! net SW at TOA [W/m^2]
!real,allocatable :: ulwtoa(:)  ! LW (upwelling) at TOA [W/m^2]
!real,allocatable :: surfsw(:)  ! net SW at surface [W/m^2]     added by Salvatore 4/9/12
!real,allocatable :: surflw(:)  ! net LW at surface [W/m^2]     added by Salvatore 4/9/12
...
!added by Yixiong and Salvatore
allocate(qconv_int(nhor)) ; qconv_int(:)=0.0
allocate(qrad_int(nhor))  ; qrad_int(:)=0.0
allocate(qconv_surf(nhor)); qconv_surf(:)=0.0
allocate(sconv_surf(nhor))     ; sconv_surf(:)=0.0
allocate(srad(nhor))      ; srad(:)=0.0
allocate(saconv(nhor))    ; saconv(:)=0.0

allocate(dswtoa(nhor))    ; dswtoa(:)=0.0
!allocate(ulwtoa(nhor))    ; ulwtoa(:)=0.0
!allocate(surflw(nhor))    ; surflw(:)=0.0    !added by Salvatore 4/9/12
!allocate(surfsw(nhor))    ; surfsw(:)=0.0    !added by Salvatore 4/9/12
...
!  write(77+mypid,'(" sin ",i2,2f10.4)') jlat,sqrt(csq(jlat)),sidalt(jlat)
enddo
...
call mpbci(ntrad   ) ! run radstep interval [steps]
call mpbci(ntconv  ) ! run dcastep interval [steps]
...
call mpbci(nxgp    ) ! 0/1 switch for 1st set of external grid point fields
call mpbci(nxcgp   ) ! 0/1 switch for 2st set of external grid point fields
call mpbci(nrad    ) ! 0/1 switch for radiative heating
call mpbci(nconv   ) ! 0/1 switch for convective adjustment
call mpbci(nheats  ) ! 0/1 switch for spectral heating
call mpbci(nheatgp ) ! 0/1 switch for gridpoint heating
...
!     initialize grid point heating

if (nheatgp > 0) then
  allocate(gtt(nhor,nlev)) 
  allocate(radtt(nhor,nlev))
!  allocate(dgtsurf(nhor))
  allocate(gmu_diag(nhor)) 
  gtt(:,:) = 0.0  
  radtt(:,:) = 0.0
!  dgtsurf(:) = 288.0
!  allocate(dgtsurf(nhor))
...
!
if(nenergy > 0) then
 allocate(denergy(NHOR,12))
 denergy(:,:)=0.
endif
if(nentropy > 0) then
 allocate(dentropy(NHOR,12))
 dentropy(:,:)=0.
endif
...
!        Save current random number generator seed
         ...
         call put_restart_arrayi('seed',meed,nseedlen,nseedlen,1)
...
!     write gridpoint arrays
      ...
      if (allocated(dgtsurf)) then
         call mpputgp('dgtsurf',dgtsurf,nhor,1)
      endif
...
!     read scalars and full spectral arrays
      ...
      if (mypid == NROOT) then
         ...
         call get_restart_arrayi('seed',meed,nseedlen,nseedlen,1)
...
!     allocate, read and scatter gridpoint arrays
      ...
      if (mypid == NROOT) call varseek('dgtsurf',ktmp)
      call mpbci(ktmp)
      if (ktmp > 0) then 
         allocate(dgtsurf(nhor))
         call mpgetgp('dgtsurf',dgtsurf,nhor,1)
      endif
...
!     not allowed in namelists for FORTRAN versions < F2003
      ...
      , dtns    , dtrop   , dttrp   , dtzz    , dvdiff  &
      , ga      , gascon  &
      , kick    , mpstep  , nafter  , ncoeff  , nxcgp , ncu, nconv   &
      , ndel    , ndheat  , ndiag   , nxgp    , ndl     , nenergy &
      , nentropy, nextout , ngui    , nguidbg , nhelsua , nkits   &
      , nlevt   , nmonths , noutput ,  nrad   , nruido  , nrun    &
      , nselect , nspecsel, nsponge , nstep   , nstop   , nsync   &
      , ntspd   , nvg     , nwpd    , nwspini , nyears  &
      , orofac  , pac     , plarad  , pspon   , psurf   , restim  &
      , rotspd  , seed    , sid_day , sigmah  , sigmax  , sponk   &
      , syncstr , synctime, t0k     , ntrad   , ntconv &
...
!--- decide if spectral or grid point heating has to be used
      if (nhelsua > 1 .or. nxgp > 0 .or. nxcgp > 0 .or. nrad > 0) then
        nheatgp = 1
        nheats  = 0
      endif
...
!     implemented.
     ...
     &                        * real(NLEV - 1 - jlev) + zsigmin
         ...
         do jlev=1,NLEV
            ...
            sigmh(jlev) = sigmh(jlev)**2*(3-2*sigmh(jlev))
         enddo
         return     
...
!added by Yixiong 22/11/2012, output of surface temp
      jcode=505
      zhelp(:)=dgtsurf(:)
      call writegp(40,zhelp,jcode,0)
...
!--- produce temperature tendencies in spectral space  
      if (nheats > 0) then
         do jlev=1,NLEV
...
!--- produce temperature tendencies in gridpoint space
      if (nheatgp > 0) then
     
        call heatgp(zampl)  ! stt(:,:) = Newtonian cooling
         
! diag output to test, added by Yixiong and Salvatore
!        write(nud,*) 'srad:',srad(:)
!        write(nud,*) 'sconv:',sconv(:)
        if(nentropy>0) then
        dentropy(:,10)=srad(:)
        dentropy(:,11)=sconv_surf(:)
        dentropy(:,12)=saconv(:)
        endif
   
        if(nenergy>0) then
        denergy(:,10)= qconv_int(:)
        denergy(:,11)= qrad_int(:)
        denergy(:,12)= qconv_surf(:)
        endif
...
!     =================
      ...
      real :: zgr12c(NHOR,NLEV)
      !real :: zstt1(NSPP,NLEV)

!     produce gridpoint restoration field (part I)
      if (nxgp > 0) then
        zgr12(:,:) = gr1(:,:) + gr2(:,:)*zampl
      endif

!     produce gridpoint restoration field (part II)
      if (nxcgp > 0) then
        zgr12c(:,:) = gr1c(:,:) + gr2c(:,:)*zampl
      endif

!     produce spectral restoration field & transformation t
      if (nhelsua > 1) then
...
!              stop
!      endif
!     Newtonian cooling (part I)
      if (nhelsua > 1 .or. nxgp > 0) then
        gtt(:,:) = (zgr12(:,:) - gt(:,:)) * gtdamp(:,:)
      endif

!     Newtonian cooling (part II)
      if (nxcgp > 0) then
        gtt(:,:) = gtt(:,:) + (zgr12c(:,:) - gt(:,:)) * gtdampc(:,:)
      endif

!     temperature tendency due to radiative heating
      if (nrad > 0) then
        call radstep
      endif

!     temperature tendency due to convective adjustment
      if (nconv > 0 .and. mod(nstep,ntconv)==0) then
        call dcastep ! Dry Convective Adjustment
      endif

!     transform t-tendencies to spectral space  
      call gp2fc(gtt ,NLON,NLPP*NLEV)
      do jlev=1,NLEV
         call fc2sp(gtt(1,jlev),zstf(1,jlev))
      enddo
      call mpsumsc(zstf,stt,NLEV)

      !call mpsumsc(zstf,zstt1,NLEV)
